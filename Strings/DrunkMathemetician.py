...
def to_metric(a_string):
    ...
    if "cups" in a_string:     
        conv= float(newString[0]) * 240
        return round(conv,2)
    if "tablespoons" in a_string:       
        conv= float(newString[0]) * 14.79
        return round(conv,2)
    if "gallons" in a_string:
        conv= float(newString[0]) * 3785.41
        return round(conv,2)
    if "pints" in a_string:
        conv= float(newString[0]) * 473.18
        return round(conv,2)
    if "quarts" in a_string:
        conv= float(newString[0]) * 946.35
        return round(conv,2)
    if "ounces" in a_string:
        conv= float(newString[0]) * 29.57
        return round(conv,2)
    if "teaspoons" in a_string:
        conv= float(newString[0]) * 4.93
        return round(conv,2)
