...
def reader(filename):
    ...
    for item in readfile:
        ...
        item0 = int(splititem[0])
        item2 = int(splititem[2])
        item3 = int(splititem[3])
        item4 = float(splititem[4])
        
        result = item0,splititem[1],item2, item3,item4
        list.append(result)
    
    dictionary ={}
    for items in list:
        dict = {"total":items[3],"number":items[0],"grade":items[2], "weight":items[4]}
        dictionary[items[1]]=dict
    return dictionary
