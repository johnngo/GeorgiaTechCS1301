...
def count_types(a_string):
    ...
    for item in a_string:
        if item.isupper():
            if "upper" not in a_string:
                dictionary["upper"] = 1
            else:
                dictionary["upper"] += 1
                
        if item.islower():
            if "lower" not in a_string:
                dictionary["lower"] = 1
            else:
                dictionary["lower"]+=1
        
        if item.isnumeric():
            if "numeral" not in a_string:
                dictionary["numeral"] = 1
            else:
                dictionary["numeral"] += 1
        if item.isspace():
            if "space" not in a_string:
                dictionary["space"] = 1
            else:
                dictionary["space"] += 1
        if item in characters or item in character:
            if "punctuation" not in a_string:
...
#We create an empty dictionary where we add on the needed key
#and values from the string input.
